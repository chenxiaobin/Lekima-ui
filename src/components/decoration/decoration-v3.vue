<template>
  <div class="le-decoration-v3" ref="RefSvgV3">
    ...
    <svg :viewBox="svgViewBox" xmlns="http://www.w3.org/2000/svg" :style="containerStyle">
      <defs>
        <g id="a-side">
          <rect x="0" y="0" :width="rectW" :height="rectH" transform="skewX(43)"></rect>
        </g>
...
export default {
  ...
  mounted () {
    this.$nextTick(() => {
      this.svgWidth = (this.$refs.RefSvgV3 && this.$refs.RefSvgV3.offsetWidth) || 0
      this.svgHeight = (this.$refs.RefSvgV3 && this.$refs.RefSvgV3.offsetHeight) || 0

      this.svgViewBox = this.getSvgViewBox()

      this.outerLine = this.getRectPointStr(this.getRectPoint())
      this.innerLine = this.getRectPointStr(this.getRectPoint(10))
  ...
  methods: {
    ...
    getRectPoint (innerSize) {
      ...
      let rightTopP3 = { x: xMax - 100, y: -yMax + diffLen + innerSize }
      let rightTopP4 = { x: xMax - diffLen - 100, y: -yMax + innerSize }
    ...
    getUsePosition () {
      let data = []
      for (let i = 0; i < 4; i++) {
        let x = (this.rectW + 8) * i + this.svgWidth / 2 - 105
        let y = -this.svgHeight / 2 + this.rectH / 2
        let color = this.getColorOpacity(this.innerStrokeColor, 4, i)
        data.push({
          id: '#a-side',
          x: x,
          y: y,
